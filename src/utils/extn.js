// 最常用的公用方法，放到obpm对象中全局挂载，方便使用
import store from '@/store'
import dayjs from 'dayjs'
import context from '@/main'
import define from '@/utils/define'
const STORAGEPREFIX = 'extn_'
...
const extn = {
    ...
    getDate:(format, strInterval, number)=> {
        ...
        return obpm.toDate(myDate, format);
    },
    toDate: (v, format)=> {
        ...
        if (childNode.length > 0) {
            for (let i = 0; i < childNode.length; i++) {
                let treeModel = {
                    ...
                    ChildNodes: obpm.toTreeViewJson(data, childNode[i][idText], parentIdText, idText),
    ...
    toFileSize: (size)=> {
        ...
        if (size < 1024.00)
            return obpm.toDecimal(size) + " 字节";
        else if (size >= 1024.00 && size < 1048576)
            return obpm.toDecimal(size / 1024.00) + " KB";
        else if (size >= 1048576 && size < 1073741824)
            return obpm.toDecimal(size / 1024.00 / 1024.00) + " MB";
        else if (size >= 1073741824)
            return obpm.toDecimal(size / 1024.00 / 1024.00 / 1024.00) + " GB";
    },
    toDecimal:(num)=> {
        if (num == null) {
            num = "0";
